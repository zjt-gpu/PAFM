...
from Models.PAFM.model_utils import unnormalize_to_zero_to_one
...
def parse_args():
    ...
    parser.add_argument('--config_file', type=str, default='Config/mujoco.yaml', 
...
def main():
    ...
    if args.train:
        ...
        np.save(os.path.join(args.save_dir, f'flow_{args.mode}_{args.name}.npy'), samples)
    else:
        trainer.load(args.milestone)
        ...
        if dataset.auto_norm:
            samples = unnormalize_to_zero_to_one(samples)
        np.save(os.path.join(args.save_dir, f'flow_fake_{args.name}.npy'), samples)
